# E6 figures

`architecture.svg` and `architecture.png` at the repository root describe the current E6 experiment. `e6_results.svg` and `e6_results.png` display the saved E6 development aggregate. The completed Original/E6 comparison uses strict scoring. Original's logic score is zero because all 100 outputs omit the required wrapper; a separate diagnostic recovers 29 correct bare answers. The figure labels this limitation.

To rebuild (optional plotting dependency):

```bash
python -m pip install matplotlib
python figures/render_figures.py
```

The renderer reads `results/e6_custom_v1/e6_update100_summary.json` and, when present, a completed audited `matched_comparison.json`. It never infers completion from partial counts. `architecture.mmd` is an editable semantic flow; the SVG/PNG layout is generated by the Python source.

SVG exports omit timestamps and use stable identifiers, so repeated builds in the same plotting environment preserve file hashes.
