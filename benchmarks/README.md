...
The strict comparison includes output-format failures. See the [format diagnostic](../results/README.md#result-interpretation) before interpreting Original's logic score.
...
## Historical MATH-500 transfer protocol
...
This shared transfer protocol differs from published Qwen benchmark settings. Cross-paper scores do not provide a matched comparison. Base-model pretraining contamination is unknown. The September 7 plan required completion and replay of all three arms before publication, including any losses relative to Original or answer-SFT. This archived plan is not a current queue-status claim.
