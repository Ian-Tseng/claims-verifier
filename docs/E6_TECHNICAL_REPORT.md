...
The current limited-disclosure release includes task inputs and gold answers for 1,500 training tasks and 300 development tasks, task definitions and aggregate results. It excludes complete method-specific training records, representation-training targets, method-specific prompts and operator codebooks, reference maps, executors, full training infrastructure, weights and raw responses. New tests on this task content must disclose their own prompts and scoring contract; they do not reproduce the reported experiment merely by sharing task IDs. New transfer experiments and collaborator-provided material remain outside the release.
